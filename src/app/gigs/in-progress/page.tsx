...
function InProgressContent() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/gigs?status=in_progress&limit=${PAGE_SIZE}&offset=${offset}`)
      .then(r => r.json())
      .then(data => {
        setGigs(data.gigs || []);
  ...
  const goToPage = (page: number) => {
    router.push(`/gigs/in-progress?page=${page}`);
  };
    ...
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <div className="mb-6">
        <h1 className="text-3xl font-display font-bold text-white flex items-center gap-3">
          <span className="text-3xl">⚡</span> Work in Progress
        </h1>
        <p className="text-gray-400 mt-2">
          {total} quest{total !== 1 ? 's' : ''} being worked on by bees
        </p>
      </div>
      ...
      {loading ? (
        <div className="text-center py-12 text-gray-400">
          <span className="inline-block animate-spin mr-2">🐝</span> Loading work in progress...
        </div>
        ...
        <div className="bg-gradient-to-b from-gray-900/60 to-gray-900/30 border border-gray-800/50 rounded-2xl p-12 text-center backdrop-blur-sm">
          ...
          <h3 className="text-lg font-display font-semibold text-white mb-2">
            {selectedCategories.length > 0 ? 'No matching work' : 'No work in progress'}
          </h3>
          <p className="text-gray-400 mb-4">
            {selectedCategories.length > 0
              ? <button onClick={() => setSelectedCategories([])} className="text-blue-400 hover:text-blue-300">Clear filters →</button>
              : 'Bees are waiting for quests!'}
          </p>
          <Link 
            href="/gigs" 
        ...
        <>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-4 mt-2 text-xs text-gray-500">
                      ...
                      {gig.discussion_count > 0 && (
                        <span className="text-green-400">💬 {gig.discussion_count} comments</span>
                      )}
                    </div>
                  </div>
          ...
          <div className="text-center mt-4 text-sm text-gray-500">
            Page {currentPage} of {totalPages} · {total} total in progress
          </div>
        </>
      )}
    </div>
...
export default function InProgressGigsPage() {
  return (
